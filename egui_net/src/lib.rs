   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    .with(EguiFn::egui_extras_table_TableBuilder_show, |mut ui: EguiPointer<Ui>, cfg: SerializableTableBuilder, header_height: Option<f32>, header_callback: Option<EguiCallback>, body_callback: EguiCallback| unsafe {
        ...
        let result = if let (Some(height), Some(header_cb)) = (header_height, header_callback) {
            ...
            table.body(|body| {
                let mut handle = TableBodyHandle { body: ManuallyDrop::new(body), consumed: false };
                body_callback.invoke(&mut handle as *mut TableBodyHandle as *const _);
            })
        }
        else {
            builder.body(|body| {
    ...
    .with(EguiFn::egui_extras_table_TableBody_row, |mut handle: EguiPointer<TableBodyHandle<'_>>, height: f32, callback: EguiCallback| unsafe {
        let handle = handle.get_mut();
        assert!(!handle.consumed, "TableBody was already consumed by Rows/HeterogeneousRows");
        handle.body.row(height, |mut row| {
            callback.invoke(&mut row as *mut TableRow as *const _);
        });
    })
    .with(EguiFn::egui_extras_table_TableBody_rows, |mut handle: EguiPointer<TableBodyHandle<'_>>, row_height: f32, total_rows: usize, callback: EguiCallback| unsafe {
        let handle = handle.get_mut();
        assert!(!handle.consumed, "TableBody was already consumed by Rows/HeterogeneousRows");
        let body = ManuallyDrop::take(&mut handle.body);
        handle.consumed = true;
        body.rows(row_height, total_rows, |mut row| {
            callback.invoke(&mut row as *mut TableRow as *const _);
        });
    })
    .with(EguiFn::egui_extras_table_TableBody_heterogeneous_rows, |mut handle: EguiPointer<TableBodyHandle<'_>>, heights: Vec<f32>, callback: EguiCallback| unsafe {
        let handle = handle.get_mut();
        assert!(!handle.consumed, "TableBody was already consumed by Rows/HeterogeneousRows");
        let body = ManuallyDrop::take(&mut handle.body);
        handle.consumed = true;
...
/// Wraps a [`TableBody`] so that it may be shared with C# for the duration of a `body`
/// callback. [`TableBody::row`] takes `&mut self` and may be called any number of times
/// through this handle, but [`TableBody::rows`]/[`TableBody::heterogeneous_rows`] consume
/// `self` by value; when one of those is invoked, `body` is taken out of the [`ManuallyDrop`]
/// (see `egui_extras_table_TableBody_rows`/`_heterogeneous_rows`) and `consumed` is set so
/// that this type's [`Drop`] impl does not run [`TableBody`]'s destructor a second time.
struct TableBodyHandle<'a> {
    body: ManuallyDrop<TableBody<'a>>,
    consumed: bool
}

impl Drop for TableBodyHandle<'_> {
    fn drop(&mut self) {
        if !self.consumed {
