...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    .with(EguiFn::egui_plot_plot_Plot_show, |mut ui: EguiPointer<Ui>, plot: SerializablePlot, callback: EguiCallback| unsafe {
        let result = egui_plot::Plot::from(plot).show(ui.get_mut(), |plot_ui| callback.invoke(plot_ui as *const _ as *const _));
        (result.response, result.transform, result.hovered_plot_item)
    })
    .with(EguiFn::egui_plot_plot_PlotUi_line, |mut plot_ui: EguiPointer<PlotUi>, line: SerializableLine| unsafe {
        plot_ui.get_mut().line(egui_plot::Line::from(line));
    })
    .with(EguiFn::egui_plot_plot_PlotUi_points, |mut plot_ui: EguiPointer<PlotUi>, points: SerializablePoints| unsafe {
        plot_ui.get_mut().points(egui_plot::Points::from(points));
    })
    // Miscellaneous
...
/// The wire format for [`egui_plot::Line`].
///
/// `PlotPoints` can't cross the FFI boundary either - its `Generator` variant stores a closure -
...
impl From<SerializableLine> for egui_plot::Line<'static> {
    fn from(value: SerializableLine) -> Self {
        let mut line = egui_plot::Line::new(value.name, egui_plot::PlotPoints::new(value.points));
        ...
        if let Some(fill_alpha) = value.fill_alpha {
            line = line.fill_alpha(fill_alpha);
        }
...
/// The wire format for [`egui_plot::Points`]. See [`SerializableLine`] for why this carries a
/// plain point list rather than a `PlotPoints`.
#[derive(Clone, Serialize, Deserialize)]
struct SerializablePoints {
    name: String,
    points: Vec<[f64; 2]>,
...
impl From<SerializablePoints> for egui_plot::Points<'static> {
    fn from(value: SerializablePoints) -> Self {
        let mut points =
            egui_plot::Points::new(value.name, egui_plot::PlotPoints::new(value.points));
