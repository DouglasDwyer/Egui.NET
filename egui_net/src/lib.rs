   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    .with(EguiFn::egui_extras_table_TableBuilder_show, |mut ui: EguiPointer<Ui>, cfg: SerializableTableBuilder, header_height: Option<f32>, header_callback: Option<EguiCallback>, body_callback: EguiCallback| unsafe {
        ...
        for column in &cfg.columns {
            builder = builder.column(column.to_column());
        }
        ...
        let result = if let (Some(height), Some(header_cb)) = (header_height, header_callback) {
            ...
            table.body(|mut body| {
                body_callback.invoke(&mut body as *mut TableBody as *const _);
            })
        }
        else {
    ...
    .with(EguiFn::egui_extras_table_TableBody_row, |mut body: EguiPointer<TableBody<'_>>, height: f32, callback: EguiCallback| unsafe {
        body.get_mut().row(height, |mut row| {
    ...
    .with(EguiFn::egui_extras_table_TableRow_set_selected, |mut row: EguiPointer<TableRow<'_, '_>>, selected: bool| unsafe { row.get_mut().set_selected(selected); })
    .with(EguiFn::egui_extras_table_TableRow_set_hovered, |mut row: EguiPointer<TableRow<'_, '_>>, hovered: bool| unsafe { row.get_mut().set_hovered(hovered); })
    .with(EguiFn::egui_extras_table_TableRow_set_overline, |mut row: EguiPointer<TableRow<'_, '_>>, overline: bool| unsafe { row.get_mut().set_overline(overline); })
    .with(EguiFn::egui_extras_table_TableRow_response, |row: EguiPointer<TableRow<'_, '_>>| unsafe { row.get().response() })
...
pub struct SerializableColumn {
    ...
    at_least: Option<f32>,
    at_most: Option<f32>,
    auto_size_this_frame: Option<bool>
}

impl SerializableColumn {
    /// Converts this to a usable [`Column`].
    fn to_column(&self) -> Column {
        let mut result = match self.kind {
            0 => Column::auto_with_initial_suggestion(self.width),
            1 => Column::initial(self.width),
            2 => Column::exact(self.width),
            _ => Column::remainder()
        };

        if let Some(resizable) = self.resizable {
            result = result.resizable(resizable);
        }
        if let Some(clip) = self.clip {
            result = result.clip(clip);
        }
        if let Some(at_least) = self.at_least {
            result = result.at_least(at_least);
        }
        if let Some(at_most) = self.at_most {
            result = result.at_most(at_most);
        }
        if let Some(auto_size_this_frame) = self.auto_size_this_frame {
            result = result.auto_size_this_frame(auto_size_this_frame);
        }
...
pub struct SerializableTableBuilder {
    ...
    columns: Vec<SerializableColumn>,
    striped: Option<bool>,
