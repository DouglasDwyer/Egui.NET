...
use egui::util::id_type_map::{RawKey, RawValue, TypeId};
...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    // `Context::data` is read-only, but egui's `Context` is internally lock-based anyway, so
    // (mirroring `Context::memory`/`memory_mut` above) both read and read-write C# access go
    // through `data_mut`.
    .with(EguiFn::egui_context_Context_data_mut, |ctx: EguiPointer<Context>, callback: EguiCallback| unsafe {
        ctx.get().data_mut(|d| callback.invoke(d as *mut _ as *const _))
    })
    // Bindings for IdTypeMap
    //
    // A `dyn Any` value can't cross the C# FFI boundary (there is no C#-side concrete type it
    // could ever downcast to), so `get_temp_raw`/`get_temp_raw_mut`/`remove_temp_raw` return the
    // value's `TypeId` wrapped in a `RawValue` instead of the value itself.
    .with(EguiFn::egui_util_id_type_map_IdTypeMap_get_temp_raw, |map: EguiPointer<IdTypeMap>, raw: RawKey| unsafe {
        map.get().get_temp_raw(raw).map(|value| RawValue { type_id: value.type_id().into() })
    })
    .with(EguiFn::egui_util_id_type_map_IdTypeMap_get_temp_raw_mut, |mut map: EguiPointer<IdTypeMap>, raw: RawKey| unsafe {
        map.get_mut().get_temp_raw_mut(raw).map(|value| RawValue { type_id: value.type_id().into() })
    })
    .with(EguiFn::egui_util_id_type_map_IdTypeMap_remove_temp_raw, |mut map: EguiPointer<IdTypeMap>, raw: RawKey| unsafe {
        map.get_mut().remove_temp_raw(raw).map(|value| RawValue { type_id: value.type_id().into() })
    })
    .with(EguiFn::egui_util_id_type_map_IdTypeMap_temp_keys, |map: EguiPointer<IdTypeMap>| unsafe {
        map.get().temp_keys().collect::<Vec<_>>()
    })
    // `RawKey::new` is generic over a Rust `T: 'static`, which can't be supplied from C# - see
    // `RawKey::from_type_id`'s doc comment for why a plain `TypeId` argument works just as well.
    .with(EguiFn::egui_util_id_type_map_RawKey_new, |type_id: TypeId, id: Id| RawKey::from_type_id(type_id, id))
