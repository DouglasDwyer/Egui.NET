...
const EGUI_FNS: EguiFnMap = AUTOGENERATED_EGUI_FNS
   ...
   .with(EguiFn::egui_containers_collapsing_header_CollapsingState_show_toggle_button, |mut state: CollapsingState, mut ui: EguiPointer<Ui>, callback: EguiCallback| unsafe {
    ...
    // `CCursor::new` is redundant: `CharIndex` is a `#[serde(transparent)]` newtype around
    // `usize`, so `CCursor.Index` is already a plain `ulong` field in C#, and `PreferNextRow`
    // already defaults to `false` - `new CCursor { Index = ... }` does the same thing with no
    // FFI round-trip needed.
    ...
    // Bindings for ByteIndex/CharIndex/ByteRange/CharRange (see egui::epaint::text::index)
    // These are `#[serde(transparent)]` newtypes around `usize`, so the autobinder doesn't see
    // them as distinct named types; bound concretely against plain integers instead.
    .with(EguiFn::epaint_text_index_ByteIndex_default, || ByteIndex::default().0 as u64)
    .with(EguiFn::epaint_text_index_ByteIndex_saturating_add, |index: u64, rhs: u64| ByteIndex(index as usize).saturating_add(rhs as usize).0 as u64)
    .with(EguiFn::epaint_text_index_ByteIndex_saturating_sub, |index: u64, rhs: u64| ByteIndex(index as usize).saturating_sub(rhs as usize).0 as u64)
    .with(EguiFn::epaint_text_index_CharIndex_default, || CharIndex::default().0 as u64)
    .with(EguiFn::epaint_text_index_CharIndex_saturating_add, |index: u64, rhs: u64| CharIndex(index as usize).saturating_add(rhs as usize).0 as u64)
    .with(EguiFn::epaint_text_index_CharIndex_saturating_sub, |index: u64, rhs: u64| CharIndex(index as usize).saturating_sub(rhs as usize).0 as u64)
    .with(EguiFn::epaint_text_index_ByteRange_full, |text: String| { let r = ByteRange::full(&text); (r.start.0 as u64, r.end.0 as u64) })
    .with(EguiFn::epaint_text_index_ByteRange_slice, |text: String, start: u64, end: u64| (ByteIndex(start as usize)..ByteIndex(end as usize)).slice(&text).to_string())
    .with(EguiFn::epaint_text_index_CharRange_full, |text: String| { let r = CharRange::full(&text); (r.start.0 as u64, r.end.0 as u64) })
    // Bindings for LayoutJob/Row/PlacedRow (precise text-index helpers, bound against plain
    // integers for the same reason as ByteIndex/CharIndex above)
    .with(EguiFn::epaint_text_text_layout_types_LayoutJob_format_at_byte, |job: LayoutJob, byte_idx: u64| job.format_at_byte(ByteIndex(byte_idx as usize)).clone())
    .with(EguiFn::epaint_text_text_layout_types_Row_char_at, |row: Row, desired_x: f32| row.char_at(desired_x).0 as u64)
    .with(EguiFn::epaint_text_text_layout_types_Row_char_count_excluding_newline, |row: Row| row.char_count_excluding_newline().0 as u64)
    .with(EguiFn::epaint_text_text_layout_types_Row_x_offset, |row: Row, column: u64| row.x_offset(CharIndex(column as usize)))
    .with(EguiFn::epaint_text_text_layout_types_PlacedRow_char_count_including_newline, |row: PlacedRow| row.char_count_including_newline().0 as u64)
    // Bindings for text_selection::text_cursor_state (free functions; bound against plain
    // integers for the same reason as ByteIndex/CharIndex above)
    .with(EguiFn::egui_text_selection_text_cursor_state_byte_index_from_char_index, |s: String, char_index: u64| byte_index_from_char_index(&s, CharIndex(char_index as usize)).0 as u64)
    .with(EguiFn::egui_text_selection_text_cursor_state_char_index_from_byte_index, |s: String, byte_index: u64| char_index_from_byte_index(&s, ByteIndex(byte_index as usize)).0 as u64)
    .with(EguiFn::egui_text_selection_text_cursor_state_slice_char_range, |s: String, start: u64, end: u64| slice_char_range(&s, CharIndex(start as usize)..CharIndex(end as usize)).to_string())
    // Bindings for Frame
    ...
    // Bindings for RichText
    // `variation` takes `impl IntoTag`, which the autobinder can't handle generically; bound
    // concretely against `String` (`&str` implements `IntoTag`) instead.
    .with(EguiFn::egui_widget_text_RichText_variation, |obj: RichText, tag: String, coord: f32| obj.variation(tag.as_str(), coord))
    // Additional `variation` overloads for the other `IntoTag` implementors (`u32` and `[u8; 4]`).
    .with(EguiFn::egui_widget_text_RichText_variation_u32, |obj: RichText, tag: u32, coord: f32| obj.variation(tag, coord))
    .with(EguiFn::egui_widget_text_RichText_variation_bytes, |obj: RichText, tag: [u8; 4], coord: f32| obj.variation(tag, coord))
    ...
    // Bindings for Ui
    // `InteractOptions` is a single-field (`move_to_top: bool`) struct; rather than exposing it
    // as its own type, `interact_opt` is bound directly against that one bool.
    .with(EguiFn::egui_ui_Ui_interact_opt, |ui: EguiPointer<Ui>, rect: Rect, id: Id, sense: Sense, move_to_top: bool| unsafe {
        ui.get().interact_opt(rect, id, sense, InteractOptions { move_to_top })
    })
