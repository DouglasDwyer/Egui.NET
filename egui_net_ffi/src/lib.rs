...
impl EguiFfi {
    ...
    pub fn full_output(&self) -> FullOutput {
        bincode::serde::decode_from_slice::<FullOutput2, _>(
            &self.full_output,
            bincode::config::legacy(),
        )
        .expect("Failed to deserialize FullOutput")
        .0
        .into()
    }
    ...
    pub fn set_full_output(&mut self, full_output: FullOutput) {
        self.full_output = bincode::serde::encode_to_vec(
            &FullOutput2::from(full_output),
            bincode::config::legacy(),
        )
        .expect("Failed to serialize FullOutput")
        .into();
    }
