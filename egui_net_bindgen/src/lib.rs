...
const BINDING_EXCLUDE_FNS: &[&str] = &[
    ...
    // `RawKey::new` is generic over a Rust `T: 'static`, which the autobinder can't resolve to
    // any C# type - bound manually against a plain `TypeId` argument instead (`RawKey::from_type_id`)
