...
/// C# names of every primitive value type that [`BindingsGenerator::bound_ty_name`] can
/// produce. Used by [`BindingsGenerator::emit_aot_roots`] to tell a `T?` meaning
/// `System.Nullable<T>` apart from a nullable-reference annotation on `string`.
const PRIMITIVE_VALUE_TYPE_NAMES: &[&str] = &[
    "bool", "byte", "sbyte", "short", "ushort", "int", "uint", "long", "ulong",
    "nint", "nuint", "Int128", "UInt128", "float", "double", "char"
];

/// The names of `EguiMarshal.AotRoot`'s fixed-size-array-rooting methods, indexed by
/// array length minus 2 (i.e. `ARRAY_SHAPE_NAMES[0]` is for `Array2<T>`).
const ARRAY_SHAPE_NAMES: &[&str] = &["Array2", "Array3", "Array4", "Array5", "Array6"];

/// The names of `EguiMarshal.AotRoot`'s tuple-rooting methods, indexed by tuple arity
/// minus 2 (i.e. `TUPLE_SHAPE_NAMES[0]` is for a 2-element tuple).
const TUPLE_SHAPE_NAMES: &[&str] = &["Tuple2", "Tuple3", "Tuple4"];
...
pub struct BindingsGenerator {
    ...
    output_path: PathBuf,
    /// The directory containing hand-written C# source files that are compiled
    /// alongside the generated bindings (i.e. the `Egui/` project directory).
    hand_written_path: PathBuf,
...
impl BindingsGenerator {
    ...
    ///
    /// `hand_written_path` should point to the directory of hand-written C# source
    /// files (the `Egui/` project) that get compiled alongside the generated
    /// bindings - it is scanned, in addition to `path`, when determining which
    /// closed generic instantiations to root for Native AOT (see [`Self::emit_aot_roots`]).
    pub fn generate(path: &std::path::Path, hand_written_path: &std::path::Path) {
        let mut krate = serde_json::from_str::<Crate>(include_str!(concat!(env!("OUT_DIR"), "/egui.json"))).expect("Failed to parse egui");
        ...
        BindingsGenerator {
            ...
            hand_written_path: hand_written_path.to_path_buf(),
    ...
    fn run(mut self) {
        ...
        generator.write_source_files(self.output_path.clone(), &self.registry).expect("Failed to write source files");

        self.emit_aot_roots();
    }

    /// `EguiMarshal.SerializerCache<T>` dispatches to a small, fixed set of compound
    /// generic wrapper serializers (`ImmutableArray<T>`, `T?`/`Nullable<T>`, tuples,
    /// `ArrayN<T>`) via `MethodInfo.MakeGenericMethod`. Native AOT cannot create new
    /// generic instantiations at runtime, so every closed instantiation that the bindings
    /// use has to be referenced *ordinarily* (not through reflection) somewhere in the
    /// compiled program ahead of time.
    ///
    /// This scans every generated (`self.output_path`) and hand-written (`self.hand_written_path`)
    /// C# file for `EguiMarshal.Call<...>` and `EguiMarshal.SerializerCache<...>` usages,
    /// recursively decomposes each compound generic argument, and emits a companion
    /// `AotRoots.g.cs` file with a module initializer that ordinarily references every
    /// instantiation found - forcing the AOT compiler to compile them ahead of time.
    fn emit_aot_roots(&self) {
        let mut value_type_names: HashSet<String> = PRIMITIVE_VALUE_TYPE_NAMES.iter().map(|x| x.to_string()).collect();

        let mut sources = Vec::new();
        for path in Self::all_cs_files(&self.output_path) {
            sources.push(std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("Failed to read {}: {e}", path.display())));
        }
        for path in Self::all_cs_files(&self.hand_written_path) {
            sources.push(std::fs::read_to_string(&path).unwrap_or_else(|e| panic!("Failed to read {}: {e}", path.display())));
        }

        let mut plain_usings: BTreeSet<String> = BTreeSet::new();
        let mut aliases: BTreeMap<String, String> = BTreeMap::new();
        for source in &sources {
            Self::gather_value_type_names(source, &mut value_type_names);
            Self::gather_usings(source, &mut plain_usings, &mut aliases);
            // `EguiFn.g.cs` declares many separate `namespace Egui.Foo { ... }` blocks in a
            // single file (one per extended type), so unqualified names inside them (like
            // `HexColor` inside `namespace Egui.Ecolor { ... }`) resolve through namespace
            // nesting rather than an explicit `using`. Treat every declared namespace as an
            // implicit using too, so copied type text keeps resolving the same way.
            Self::gather_namespace_declarations(source, &mut plain_usings);
        }

        let mut roots: BTreeSet<(&'static str, Vec<String>)> = BTreeSet::new();
        for source in &sources {
            for prefix in ["EguiMarshal.Call<", "EguiMarshal.SerializerCache<"] {
                let mut search_from = 0;
                while let Some(relative_start) = source[search_from..].find(prefix) {
                    let args_start = search_from + relative_start + prefix.len();
                    let args_end = Self::find_matching_angle_bracket(source, args_start)
                        .unwrap_or_else(|| panic!("Unbalanced generic arguments after `{prefix}` at byte {args_start}"));

                    for argument in Self::split_top_level(&source[args_start..args_end], ',') {
                        Self::collect_aot_roots(argument.trim(), &value_type_names, &mut roots);
                    }
        ...
        let mut using_aliases = String::new();
        for using in &plain_usings {
            using_aliases.push_str(&format!("using {using};\n"));
        }
        for (name, target) in &aliases {
            using_aliases.push_str(&format!("using {name} = {target};\n"));
        }

        let mut body = String::new();
        for (shape, args) in &roots {
            body.push_str(&format!("        global::Egui.EguiMarshal.AotRoot.{shape}<{}>();\n", args.join(", ")));
        }
...
// that the generated and hand-written bindings actually use, so that Native AOT compiles
// them ahead of time instead of only ever discovering them through reflection. See the
// remarks on EguiMarshal.SerializerCache<T> and EguiMarshal.AotRoot for details.
//
// Local `using` aliases are replicated below (rather than relying on the project's global
// usings) because the type arguments were copied verbatim from other files that may declare
...
");
        ...
        std::fs::write(self.output_path.join("AotRoots.g.cs"), result).expect("Failed to write AOT roots file");
    }

    /// Recursively decomposes a C# type expression (as it appears in a generic argument
    /// list emitted by this crate or by `serde-generate`) and records an AOT root for
    /// every compound generic wrapper shape (see [`Self::emit_aot_roots`]) found within it.
    fn collect_aot_roots(ty: &str, value_type_names: &HashSet<String>, roots: &mut BTreeSet<(&'static str, Vec<String>)>) {
        let ty = ty.trim();

        if let Some(inner) = ty.strip_suffix('?') {
            // `T?` only means `System.Nullable<T>` when `T` is a value type; for reference
            // types (in this codebase, only `string`) it's an erased nullability annotation.
            if Self::is_value_type(inner, value_type_names) {
                roots.insert(("Nullable", vec![inner.to_string()]));
                Self::collect_aot_roots(inner, value_type_names, roots);
            }
            return;
        }

        for len in 2..=6 {
            let prefix = format!("Array{len}<");
            if let Some(inner) = ty.strip_prefix(&prefix).and_then(|x| x.strip_suffix('>')) {
                roots.insert((ARRAY_SHAPE_NAMES[len - 2], vec![inner.to_string()]));
                Self::collect_aot_roots(inner, value_type_names, roots);
                return;
            }
        }

        if let Some(inner) = ty.strip_prefix("ImmutableArray<").and_then(|x| x.strip_suffix('>')) {
            roots.insert(("ImmutableArray", vec![inner.to_string()]));
            Self::collect_aot_roots(inner, value_type_names, roots);
            return;
        }

        // Tuples appear either as an explicit `ValueTuple<...>` (emitted by this crate and by
        // `serde-generate`) or as C#'s `(A, B)` tuple literal syntax (used throughout the
        // hand-written bindings) - both need to be recognized.
        let tuple_inner = ty.strip_prefix("ValueTuple<").and_then(|x| x.strip_suffix('>'))
            .or_else(|| ty.strip_prefix('(').and_then(|x| x.strip_suffix(')')));
        if let Some(inner) = tuple_inner {
            let items = Self::split_top_level(inner, ',').map(|x| x.trim().to_string()).collect::<Vec<_>>();
            if let Some(&shape) = TUPLE_SHAPE_NAMES.get(items.len().wrapping_sub(2)) {
                roots.insert((shape, items.clone()));
            }
            for item in &items {
                Self::collect_aot_roots(item, value_type_names, roots);
            }
            return;
        }

        // A plain primitive or generated struct/enum name (or an unsupported compound
        // type, e.g. `ImmutableDictionary<,>`) - not a wrapper shape `SerializerCache<T>`
        // needs to root; it either has no generic instantiation to create, or is handled
        // through its own non-generic `Serialize`/`Deserialize` methods.
    }

    /// Returns whether `name` (a bare identifier, or another compound wrapper shape) is a
    /// C# value type - i.e. whether `{name}?` means `System.Nullable<T>` rather than an
    /// erased nullable-reference annotation.
    fn is_value_type(name: &str, value_type_names: &HashSet<String>) -> bool {
        // `name` may be fully qualified (e.g. `Egui.ScrollingToTarget`) even though
        // `value_type_names` only records bare declaration names, so check both.
        let unqualified = name.rsplit('.').next().unwrap_or(name);
        name != "string"
            && (value_type_names.contains(name)
                || value_type_names.contains(unqualified)
                || name.starts_with("ImmutableArray<")
                || name.starts_with("ValueTuple<")
                || name.starts_with('(')
                || (2..=6).any(|len| name.starts_with(&format!("Array{len}<"))))
    }

    /// Collects the names of every `struct`/`enum` declared in `source` into `out`. Used to
    /// tell apart `T?` meaning `Nullable<T>` (value types) from a nullable-reference
    /// annotation (reference types) when scanning for AOT roots.
    fn gather_value_type_names(source: &str, out: &mut HashSet<String>) {
        for keyword in [" struct ", " enum "] {
            let mut search_from = 0;
            while let Some(relative_start) = source[search_from..].find(keyword) {
                let name_start = search_from + relative_start + keyword.len();
                let name_len = source[name_start..].find(|c: char| !c.is_alphanumeric() && c != '_').unwrap_or(source.len() - name_start);
                if name_len > 0 {
                    out.insert(source[name_start..name_start + name_len].to_string());
                }
    ...
    /// Collects every top-level `using` directive declared in `source` - both plain
    /// namespace imports (`using System.Collections.Immutable;`, `using static Foo;`, into
    /// `plain`) and `using Name = Target;` aliases (into `aliases`, panicking if the same
    /// alias name is ever mapped to two different targets across the scanned files, which
    /// would make copying type text between files unsound). Local (non-`global`) directives
    /// only apply within their declaring file, so they need to be replicated in any other
    /// file that reuses type text copied from it.
    fn gather_usings(source: &str, plain: &mut BTreeSet<String>, aliases: &mut BTreeMap<String, String>) {
        for line in source.lines() {
            let line = line.trim();
            let Some(rest) = line.strip_prefix("using ") else { continue };
            let Some(rest) = rest.strip_suffix(';') else { continue };

            let Some((name, target)) = rest.split_once('=') else {
                plain.insert(rest.trim().to_string());
                continue;
            };
            let (name, target) = (name.trim().to_string(), target.trim().to_string());

            // Skip `using var x = ...;`/`using (var x = ...)` disposable-scope statements,
            // which aren't using directives - a real alias's left-hand side is always a
            // single identifier.
            ...
            if let Some(existing) = aliases.get(&name) {
                assert_eq!(existing, &target, "Conflicting `using {name} = ...` alias definitions: `{existing}` vs `{target}`");
            }
            else {
    ...
    /// Collects the name of every `namespace X { ... }` / `namespace X;` declared in
    /// `source` into `out`.
    fn gather_namespace_declarations(source: &str, out: &mut BTreeSet<String>) {
        let mut search_from = 0;
        while let Some(relative_start) = source[search_from..].find("namespace ") {
            let name_start = search_from + relative_start + "namespace ".len();
            let name_end = source[name_start..].find(|c: char| !(c.is_alphanumeric() || c == '_' || c == '.')).map(|x| name_start + x).unwrap_or(source.len());
            if name_end > name_start {
                out.insert(source[name_start..name_end].to_string());
            }
            search_from = name_end.max(name_start + 1);
        }
    }

    /// Finds the index of the `>` that closes the generic argument list starting at `start`
    /// (the index just after the opening `<`), accounting for nested `<...>` pairs.
    ...
    /// Splits `source` on occurrences of `separator` that are not nested inside `<...>`,
    /// `(...)`, or `[...]`.
    fn split_top_level(source: &str, separator: char) -> impl Iterator<Item = &str> {
        ...
        for (offset, ch) in source.char_indices() {
            match ch {
                ...
                c if c == separator && depth == 0 => {
                    result.push(&source[segment_start..offset]);
        ...
        if segment_start <= source.len() {
            result.push(&source[segment_start..]);
        }
        result.into_iter().filter(|x| !x.trim().is_empty())
    }

    /// Returns a list containing all `.cs` files in `path` (and subdirectories).
    fn all_cs_files(path: &std::path::Path) -> Vec<PathBuf> {
        let mut result = Vec::new();
        ...
        for maybe_entry in entries {
            ...
            if file_type.is_dir() {
                result.extend(Self::all_cs_files(&entry.path()));
            }
            else if file_type.is_file() && entry.path().extension().is_some_and(|x| x == "cs") {
        ...
        Some(match ty {
            ...
            Type::ImplTrait(x) => if x.len() == 1
                && let Some(GenericBound::TraitBound { trait_: RdPath { path, args: trait_generics, .. }, .. }) = x.first() {
                    ...
                    else if path.contains("Hash") || path.contains("IdSource") || path.contains("AsIdSalt") {
                ...
                if let Some(ty_name) = impl_ty.name.clone() {
                    ...
                    if HANDLE_TYPES.contains(&ty_name.as_str()) {
                        ...
                        if primitive_enum {
                            ...
                            let struct_kw = if REF_STRUCT_TYPES.contains(&ty_name.as_str()) { "struct" } else { "record struct" };

                            if Self::is_ui_fn(&self.krate.index[&id]) {
                                writeln!(f, "namespace {namespace} {{ public partial {struct_kw} {display_name} : IWidget {{\n{fn_def}\n}} }}")?;
                            }
                            else {
                                writeln!(f, "namespace {namespace} {{ public partial {struct_kw} {display_name} {{\n{fn_def}\n}} }}")?;
                            }
                        }
                    }
